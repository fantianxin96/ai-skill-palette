...
final class PromptStore {
    static let skills: [Skill] = [
        ...
        Skill(
            ...
            prompt: "请使用 $i-shape，基于已确定的产品结构，帮我规划某一条核心任务流的页面路径。先不要写代码，也不要重新设计产品模块。请聚焦一个用户目标，梳理用户从进入、理解、操作、反馈到完成的具体步骤；列出每一步出现的页面、弹窗、空/loading/成功/失败状态、关键反馈、异常路径和容易漏掉的边界情况。最后给我一版可用于原型的任务流转。"
        ),
        Skill(
            category: "想清楚",
            stage: "开始前",
            ...
            prompt: "请使用 $i-shape，基于已经列出的产品结构或功能清单，帮我做 MVP 范围切分。先不要重新规划页面结构，也不要写代码。请明确：1. v1 必须保留的核心闭环；2. 可以合并的页面/模块；3. 可以先用手动或半自动方式代替的能力；4. 明确后置到 v1.1/v2 的功能；5. 第一版最小可验证路径；6. 第一版验收标准；7. 为了不把产品做散必须坚持的体验原则。"
        ),
        Skill(
            category: "想清楚",
            stage: "开始前",
