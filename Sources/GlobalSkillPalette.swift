...
final class PromptStore {
    static let skills: [Skill] = [
        ...
        Skill(
            ...
            prompt: "请使用 $i-shape，基于已确定的产品结构，帮我规划某一条核心任务流的页面路径。先不要写代码，也不要重新设计产品模块。请先确认或复述本次要规划的用户目标；如果目标不明确，请先问我最多 3 个关键问题。然后聚焦这一个用户目标，梳理用户从进入、理解、操作、反馈到完成的具体步骤。请列出：1. 每一步出现的页面、弹窗或局部状态；2. 用户在每一步要完成的动作；3. 系统在每一步要给出的反馈；4. 空/loading/成功/失败状态；5. 异常路径和恢复方式；6. 容易漏掉的边界情况；7. 容易让用户困惑的节点和引导建议。最后给我一版可用于原型的任务流转。"
        ),
        Skill(
            category: "想清楚",
            stage: "开始前",
